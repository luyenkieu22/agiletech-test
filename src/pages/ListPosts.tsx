...
import type { IAllPosts, IPostData, IPosts } from "../interfaces/post.interface";
...
const ListPosts: React.FC = () => {
    ...
    return (
        <div className="p-18">
            <div className="lg:flex grid gap-4 mb-8 items-center justify-between">
                ...
                <tbody>
                    {posts.length > 0 ? posts?.map((post: IPosts, index: number) => (
                        <tr key={post.id} className="text-center hover:bg-gray-100">
                            ...
                            <td className="p-3 border-t border-gray-300">{post?.description}</td>
                            <td className="p-3 border-t border-gray-300">{post?.tags.join(", ")}</td>
                            <td className="p-3 border-t border-gray-300">
                                <div className="flex items-center justify-center gap-3">
                                    <Edit2 className="cursor-pointer text-gray-500 hover:text-gray-900" size={22} onClick={() => handleOpenDialog(post)} />
                                    <Trash2 className="cursor-pointer text-gray-500 hover:text-gray-900" size={22} onClick={() => handleRemove(post.id!)} />
                                </div>
                            </td>
                        </tr>
            ...
            <div className="flex justify-end items-center mt-4 gap-4">
                <div className="flex items-center gap-2">
                    ...
                    <p className="border-gray-300 border px-2 rounded-xl">
                        {postsData?.current_page ? postsData?.current_page : 0}/{postsData?.total_page ? postsData?.total_page : 0}
                    </p>
                </div>
                <button onClick={() => setPage((prev) => prev - 1 < 1 ? (postsData?.total_page ?? 1) : prev - 1)}>
                    <SquareChevronLeft className="cursor-pointer shadow-lg text-gray-500 hover:text-gray-800" size={28} />
                </button>
                <button onClick={() => setPage((prev) => prev + 1 > (postsData?.total_page ?? 1) ? 1 : prev + 1)}>
                    <SquareChevronRight className="cursor-pointer shadow-lg text-gray-500 hover:text-gray-800" size={28} />
                </button>
            </div>
        </div>
    );
